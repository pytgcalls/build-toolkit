...
retrieve_prefix() {
  ...
  matching_tags=$(echo "$tags" | grep "${base_version//./[._]}" | head -n 1)
  ...
  case "$1" in
    ...
    msvc)
      if (is_windows); then
        ...
        append_env_path "PATH" "$MSVC_BIN"
        append_env_path "LIB" "$VS_BASE_PATH/$VS_EDITION/VC/Tools/MSVC/$MSVC_VERSION/lib/x64"
        append_env_path "LIB" "$WINDOWS_KITS_BASE_PATH/Lib/$WINDOWS_KITS_VERSION/um/x64"
        append_env_path "LIB" "$WINDOWS_KITS_BASE_PATH/Lib/$WINDOWS_KITS_VERSION/ucrt/x64"
        append_env_path "INCLUDE" "$VS_BASE_PATH/$VS_EDITION/VC/Tools/MSVC/$MSVC_VERSION/include"
